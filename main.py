...
def extract_place(page: Page) -> Place:
    name_xpath = '//div[contains(@class, "TIHn2")]//h1[contains(@class, "DUwDvf")]'
    address_xpath = '//button[@data-item-id="address"]//div[contains(@class, "fontBodyMedium")]'
    website_xpath = '//a[@data-item-id="authority"]//div[contains(@class, "fontBodyMedium")]'
    phone_number_xpath = '//button[contains(@data-item-id, "phone:tel:")]//div[contains(@class, "fontBodyMedium")]'
    reviews_count_xpath = '//div[contains(@class, "TIHn2")]//div[contains(@class, "fontBodyMedium")]//div//span//span//span[@aria-label]'
    reviews_average_xpath = '//div[contains(@class, "TIHn2")]//div[contains(@class, "fontBodyMedium")]//div//span[@aria-hidden]'
...
def scrape_places(search_for: str, total: int) -> List[Place]:
    ...
    with sync_playwright() as p:
        ...
        try:
            logging.info("Bypassing homepage and jumping directly to search...")
            ...
            logging.info("Scrolling and collecting unique leads...")
            while len(unique_urls) < total:
                ...
                logging.info(f"Unique Leads Found: {len(unique_urls)} / {total}")
                ...
                if len(unique_urls) == previously_counted:
                    ...
                    if len(unique_urls) == previously_counted:
                        logging.info("Reached the bottom of the list. No more leads found.")
                        break
                previously_counted = len(unique_urls)

            unique_urls = unique_urls[:total]
            logging.info(f"Target Acquired: Processing {len(unique_urls)} unique leads.")

            for idx, url in enumerate(unique_urls):
                try:
                    logging.info(f"Extracting Lead {idx+1} of {len(unique_urls)}...")
                    page.goto(url, timeout=60000)
                    page.wait_for_selector('//div[contains(@class, "TIHn2")]//h1[contains(@class, "DUwDvf")]', timeout=10000)
                    time.sleep(1.5)
                    place = extract_place(page)
                    if place.name:
                        places.append(place)
                except Exception as e:
                    logging.warning(f"Failed to extract lead {idx+1}: {e}")
...
def save_places_to_csv(places: List[Place], output_path: str = "result.csv", append: bool = False):
    ...
    if not df.empty:
        ...
        logging.info(f"Successfully saved {len(df)} clean leads to {output_path}!")
    else:
